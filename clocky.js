class Clocky {
  constructor(tempX, tempY, tempR, tempTimeZone, tempCity, color) {
    this.x = tempX;
    this.y = tempY; //height / 2 - 25;
    this.radius = tempR; //min(windowWidth, height - 150) / 2;
  ...
  makeHands() {
    ...
    var h =
      map(
        getHourInTimezone(hour(), this.timeZone) + norm(minute(), 0, 60),
        0,
        24,
        0,
        2 * TWO_PI
      ) - HALF_PI; //en este caso va de 0 a 24, y da dos vueltas, por lo que 2 * 2PI
    ...
    line(
      this.x,
      this.y,
      this.x + this.radius * 0.9 * cos(s),
      this.y + this.radius * 0.9 * sin(s)
    );
    strokeWeight(2);
    line(
      this.x,
      this.y,
      this.x + this.radius * 0.85 * cos(m),
      this.y + this.radius * 0.85 * sin(m)
    );
    strokeWeight(4);
    line(
      this.x,
      this.y,
      this.x + this.radius * 0.75 * cos(h),
      this.y + this.radius * 0.75 * sin(h)
    );
  }
  ...
  place() {
    ...
    textSize(20);
    text(this.city, this.x, this.y + this.radius + 35);
  }
